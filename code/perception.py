...
def perception_step(Rover):
    ...
    # TODO: 
    if (Rover.pitch > 0.8 and Rover.pitch < 358):
        return Rover
    elif (Rover.roll > 1.2 and Rover.roll < 358):
        return Rover
    ...
    #destination = np.float32([[Rover.img.shape[1]/2 , Rover.img.shape[0]/2 ], [Rover.img.shape[1]/2, Rover.img.shape[0]/2 -10],
    #                      [Rover.img.shape[1]/2 - 10 , Rover.img.shape[0] / 2 - 10], [ Rover.img.shape[1]/2 - 10 , Rover.img.shape[0]/2]])  
    ...
    # Ground map
    threshed = color_thresh(warped, (166,152,145))
    
    #Obstacle map
    threshed_obst = np.absolute(np.float32(threshed) - 1) * mask
    #plt.imshow(threshed_obst, cmap='gray')
    #Rocks map
    threshed_rock = color_thresh_rock(warped, (110,100,0),(180,150,50))
    ...
    #Obstacle map
    xpix_obst, ypix_obst = rover_coords(threshed_obst)
    #Rocks map
    ...
    obstacle_x_world, obstacle_y_world = pix_to_world(xpix_obst, ypix_obst, Rover.pos[0], 
                                    Rover.pos[1], Rover.yaw,  
                                    Rover.worldmap.shape[0], scale)
    ...
    (rover_centric_pixel_distances,angles) = to_polar_coords(xpix, ypix)
    #print("Angles gotten are:", angles)
	#print(angles)
    rover_centric_angles = angles
